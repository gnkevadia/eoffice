...
class Brand extends Model
{
    ...
    */
    public function getAll($orderby=null, $where=array(), $dynamicWhere='')
    {
        ...
        return $query->where(['brand.deleted' => 0])
                    ->select('brand.*', DB::raw('CASE WHEN brand.status = 1 THEN "Active" ELSE "Inactive" END as status'))
    ...
    */
    ...
    {
        return Brand::whereIn('id', explode(',', $ids))->update($arrUpdate);
    }
    ...
    */
    ...
    {
        return Brand::where('id', $id)->update($arrUpdate);
    }
    ...
    */
    ...
    {
        if ($id != null) {
            return Brand::where($criteria)->where('id', '<>', $id)->count();
        } else {
            return Brand::where($criteria)->count();
        }
    }
