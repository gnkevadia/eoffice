...
class BookingCancel extends Model
{
    ...
    */
    public function getOne($id, $where=array(), $dynamicWhere='')
    {
        ...
        return $query->where(['booking.deleted' => 0])
                    ->select('*', DB::raw('CASE WHEN booking.status = 1 THEN "Active" ELSE "Inactive" END as status'),'area.name as area','package.name as package','city.name as city')
    ...
    * @since Jan 2020
    */
    public function getOneByOrderId($id, $booking_otp, $booking_phone_number, $where=array(), $dynamicWhere='')
    {
        if (empty($dynamicWhere)) {
            $dynamicWhere = " 1 = 1";
        }
        ...
        return $query->where(['booking.deleted' => 0])
                    ...
                    ->join('city', 'city.id', '=', 'package_city.city_id')
                    ->join('area', 'area.id', '=', 'booking.area_id')
                    ->where(['booking.order_number' => $id, 'vcode' => $booking_otp, 'guardian_mobile' => $booking_phone_number])
                    ->whereRaw($dynamicWhere)
                    ->first();
    }
    /**
    ...
    * @since Jan 2020
    */
    public function checkOrderBooking($id, $booking_phone_number, $where=array(), $dynamicWhere='')
    {
        if (empty($dynamicWhere)) {
            $dynamicWhere = " 1 = 1";
        }
        ...
        return $query->where(['booking.deleted' => 0])
                    ...
                    ->join('city', 'city.id', '=', 'package_city.city_id')
                    ->join('area', 'area.id', '=', 'booking.area_id')
                    ->where(['booking.order_number' => $id, 'guardian_mobile' => $booking_phone_number])
                    ->whereRaw($dynamicWhere)
                    ->first();
    }
}
