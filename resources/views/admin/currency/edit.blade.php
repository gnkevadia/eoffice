        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <div class="kt-portlet">
                        ...
                        <!--begin::Form-->
                        <form id="frmAddEdit" name="frmAddEdit" action="{{ url(VIEW_INFO['url'].'/edit/'.$data->id) }}" class="form-horizontal"
                                        ...
                                        {{-- <div class="row">
                                            <div class="form-group col-md-6">
                                                ...
                                                <div class="kt-widget__media">
                                                    <input type="hidden" id="flag_exist" name="flag_exist" value="{{$data->flag}}">
                                                    @if(in_array(pathinfo($data->flag, PATHINFO_EXTENSION),array('png','jpg','jpeg','bmp')))
                                                        <img src="{{url($arrFile['path'].$arrFile['resize'].'x'.$arrFile['resize'].'/'.$data->flag)}}" alt="image">
                                                    @elseif(in_array(pathinfo($data->flag, PATHINFO_EXTENSION),array('mp4')))
                                                        <img src="{{url($arrFile['path'].$arrFile['resize'].'x'.$arrFile['resize'].'/'.str_replace(pathinfo($data->flag, PATHINFO_EXTENSION),'png',$data->media_file))}}" alt="image">
                                                    @else
                                                        <a href="{{url($arrFile['path'].$arrFile['resize'].'x'.$arrFile['resize'].'/'.$data->flag)}}">{{$data->flag}}</a>
                                                    @endif
                                                </div>
                                                @endif
                                            </div>
