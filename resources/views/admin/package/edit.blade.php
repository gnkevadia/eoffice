                            ...
                            <div class="kt-portlet__body">
                                    <div class="form-group form-group-last">
                                                ...
                                                <select name="category" id="category" class="form-control name" required>
                                                    ...
                                                    @foreach($arrCategory as $key=>$val)
                                                        @if($data->category == $val->id)
                                                            tierSelect += "<option value='{{$val->id}}' selected>{{$val->name}}</option>";
                                                        @else
                                        ...
                                        <div class="row">
                                            <div class="form-group col-md-6">
                                                <label>Heading</label>
                                                <input type="text" id="heading" name="heading" data-toggle="tooltip" title="Enter Heading" class="form-control" placeholder="Enter Heading" value="{{ $data->heading }}">
                                            </div>
                                            <div class="form-group col-md-6">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="form-group col-md-6">
                                                ...
                                                <select name="inquire" id="inquire" class="form-control name">
                                                    <option {{($data->inquire == 1 ? 'selected' : '')}} value="1">Yes</option>
                                                    <option {{($data->inquire == 0 ? 'selected' : '')}} value="0">No</option>
                                                </select>
                                            </div>
                                        </div>
                                        <div class="row">
                                        ...
                                        <div class="row">
                                            <div class="form-group col-md-6">
                                                ...
                                                <div></div>
                                                <div class="custom-file">
                                                    <input type="file" class="custom-file-input" id="customFile" name="banner_mobile">
                                                    <label class="custom-file-label" for="customFile">Choose file</label>
                                                </div>
                                                <span class="form-text text-muted">Image should be .png and size should be less than 1 MB</span>
                                                @if(isset($data->banner_mobile_image) && !empty($data->banner_mobile_image))
                                                <div class="kt-widget__media">
                                                    <input type="hidden" id="banner_mobile_exist" name="banner_mobile_exist" value="{{$data->banner_mobile_image}}">
                                                    @if(in_array(pathinfo($data->banner_mobile_image, PATHINFO_EXTENSION),array('png','jpg','jpeg','bmp')) && isset($arrFile['resize']) && !empty($arrFile['resize']))
                                                        <img src="{{url($arrFile['path'].$arrFile['resize'].'x'.$arrFile['resize'].'/'.$data->banner_mobile_image)}}" alt="image">
                                                    @elseif(in_array(pathinfo($data->banner_mobile_image, PATHINFO_EXTENSION),array('png','jpg','jpeg','bmp')))
                                                        <img src="{{url($arrFile['path'].'/'.$data->banner_mobile_image)}}" alt="image" width="100">
                                                    @elseif(in_array(pathinfo($data->banner_mobile_image, PATHINFO_EXTENSION),array('mp4')) && isset($arrFile['resize']) && !empty($arrFile['resize']))
                                                        <img src="{{url($arrFile['path'].$arrFile['resize'].'x'.$arrFile['resize'].'/'.str_replace(pathinfo($data->banner_mobile_image, PATHINFO_EXTENSION),'png',$data->banner_mobile_image))}}" alt="image">
                                                    @elseif(in_array(pathinfo($data->banner_mobile_image, PATHINFO_EXTENSION),array('mp4')))
                                                        <img src="{{url($arrFile['path'].'/'.str_replace(pathinfo($data->banner_mobile_image, PATHINFO_EXTENSION),'png',$data->banner_mobile_image))}}" alt="image" width="100">
                                                    @else
                                                        <a href="{{url($arrFile['path'].$arrFile['resize'].'x'.$arrFile['resize'].'/'.$data->banner_mobile_image)}}">{{$data->banner_mobile_image}}</a>
                                                    @endif
                                                </div>
                                                @endif
                                            </div>
                                            <div class="form-group col-md-6">
                                                ...
                                                <div class="kt-widget__media">
                                                    <input type="hidden" id="banner_exist" name="banner_exist" value="{{$data->banner_image}}">
                                                    @if(in_array(pathinfo($data->banner_image, PATHINFO_EXTENSION),array('png','jpg','jpeg','bmp')) && isset($arrFile['resize']) && !empty($arrFile['resize']))
                                                        <img src="{{url($arrFile['path'].$arrFile['resize'].'x'.$arrFile['resize'].'/'.$data->banner_image)}}" alt="image">
                                                    @elseif(in_array(pathinfo($data->banner_image, PATHINFO_EXTENSION),array('png','jpg','jpeg','bmp')))
                                                        <img src="{{url($arrFile['path'].'/'.$data->banner_image)}}" alt="image" width="100">
                                                    @elseif(in_array(pathinfo($data->banner_image, PATHINFO_EXTENSION),array('mp4')) && isset($arrFile['resize']) && !empty($arrFile['resize']))
                                                        <img src="{{url($arrFile['path'].$arrFile['resize'].'x'.$arrFile['resize'].'/'.str_replace(pathinfo($data->banner_image, PATHINFO_EXTENSION),'png',$data->banner_image))}}" alt="image">
                                                    @elseif(in_array(pathinfo($data->banner_image, PATHINFO_EXTENSION),array('mp4')))
                                                        <img src="{{url($arrFile['path'].'/'.str_replace(pathinfo($data->banner_image, PATHINFO_EXTENSION),'png',$data->banner_image))}}" alt="image" width="100">
                                                    @else
                                                        <a href="{{url($arrFile['path'].$arrFile['resize'].'x'.$arrFile['resize'].'/'.$data->banner_image)}}">{{$data->banner_image}}</a>
                                                    @endif
                                                </div>
                                                @endif
                                            </div>
                                ...
                                <div class="form-group">
                                    <label >Add Multiple Report Prices</label>
                                    ...
                                    <button type="button" id="addMore" class="btn btn-primary clear"><i title="Add City" class="fa fa-plus"></i> Add Prices</button>
                                </div>
...
<script src="{{ asset('admin/assets/js/pages/custom/packageSlug.js') }}"></script>
<script>
