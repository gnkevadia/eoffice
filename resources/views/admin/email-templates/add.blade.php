...
<div class="kt-subheader__breadcrumbs">
    ...
    <a href="{{ url('admin/email-templates') }}" class="kt-subheader__breadcrumbs-link">{{VIEW_INFO['title']}}</a>
    <span class="kt-subheader__breadcrumbs-separator"></span>
            ...
            <div class="kt-portlet">
                    ...
                    method="post">{{ csrf_field() }}
                        <div class="kt-portlet__body">
                                ...
                                <label>Template Type<span class="required"><code>*</code></span></label>
                                <div>
                                <select name="email_Template_type_id" id="email_Template_type_id" class="form-control" >
                                    <option value="">-Select Setting Type-</option>
                                        @if(isset($arrSettingtype) && !empty($arrSettingtype))
                                            @foreach($arrSettingtype as $key=>$val)
                                            <option value="{{ $val->id }}" {{ old('email_Template_type_id') == $val->id ? 'selected' : '' }} >{{ $val->name }}</option>
                                            @endforeach
                                        @endif
                                </select>
                                </div> 
                            ...
                            <div class="form-group">
                                <label>Content</label>
                                <textarea id="content" name="content" data-toggle="tooltip" title="Enter Content" class="form-control mceEditor" placeholder="Enter Content" required ></textarea>
                            </div>
...
@section('metronic_js')
<script src="{{ asset('admin/assets/js/pages/custom/email-templates.js') }}"></script>
@stop