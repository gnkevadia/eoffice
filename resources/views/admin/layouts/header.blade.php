	...
	<div class="kt-header__topbar">
		...
		<div class="kt-header__topbar-item kt-header__topbar-item--user">
			<div class="pt-3" >
				...
				<select name="switchcity" id="switchcity" class="form-control">
					<option value="">Switch City</option>
					@if(!empty(Session::get('cities')))
						@foreach(Session::get('cities') as $keyCities=>$valCities)
							<option value="{{$valCities->id}}" @if(Session::get('current_city_id') == $valCities->id) selected @endif>{{$valCities->name}}</option>
						@endforeach
					@endif
				</select>
				@endif
