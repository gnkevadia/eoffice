							...
							if (!isset($menuCategory['parent_cats'][$cat_id]))
							{
								...
								if(in_array($menuCategory['categories'][$cat_id]->routes,Session::get('routes'))){
									if(isset($menuCategory['categories'][$cat_id]->routes) && !empty($menuCategory['categories'][$cat_id]->routes)){
										$activeClass = "";
										//if(strpos($menuCategory['categories'][$cat_id]->routes,Request()->path())){
										if($menuCategory['categories'][$cat_id]->routes == "/".Request()->path()){
											$activeClass = " kt-menu__item--active ";
										}else{
											$last = explode("/", Request()->path(), 3);
											//if(strpos(Request()->path(),strtolower($menuCategory['categories'][$cat_id]->name))){
											//if(strpos($last[1],str_replace(" ","-",strtolower($menuCategory['categories'][$cat_id]->name)))){
											if($last[1] == str_replace(" ","-",strtolower($menuCategory['categories'][$cat_id]->name))){
												$activeClass = " kt-menu__item--active ";
											}
										}
										if($parent == 0){
											if(strpos($menuCategory['categories'][$cat_id]->routes,'myprofile')){
												$html .= "<li class='kt-menu__item  ".$activeClass." ' aria-haspopup='true' data-ktmenu-submenu-toggle='hover'><a class='kt-menu__link kt-menu__toggle' href='".$menuCategory['categories'][$cat_id]->routes."/".Session::get('id')."'> <i class='kt-menu__link-icon ".$menuCategory['categories'][$cat_id]->icon."'></i><span class='kt-menu__link-text'>".$menuCategory['categories'][$cat_id]->name."</span></a></li>";
											}else{
							...
							if (isset($menuCategory['parent_cats'][$cat_id]))
							{
								if(isset($menuCategory['categories'][$cat_id]->routes) && in_array($menuCategory['categories'][$cat_id]->routes,Session::get('routes'))){
									...
									if(isset($menuCategory['categories'][$cat_id]->routes) && !empty($menuCategory['categories'][$cat_id]->routes)){
										...
										if(request()->session()->get('role') == 3 && in_array($menuCategory['categories'][$cat_id]->name,array('Role & Permission', 'CMS Management'))){
											continue;
										}else{
