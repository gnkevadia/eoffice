        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <div class="kt-portlet">
                    ...
                    <!--begin::Form-->
                    <form id="frmAddEdit" name="frmAddEdit" action="{{ url(VIEW_INFO['url'] . '/add') }}" class="kt-form"
                        method="post">
                        {{ csrf_field() }}
                        <div class="kt-portlet__body">
                                    ...
                                    @if (isset($brand) && !empty($brand))
                                        @foreach ($brand as $key => $val)
                                            <option value="{{ $val['name'] }}"
                                                {{ old('brand') == $val['name'] ? 'selected' : '' }}>{{ $val['name'] }}
                                            </option>
                                        @endforeach
                                    @endif
                                </select>
                            ...
                            <div class="form-group">
                                <label for="exampleSelect1">States<span class="required"><code>*</code></span></label>
                                <select name="states" id="states" class="form-control">
                                    <option value="">No States</option>
                                    @if (isset($states) && !empty($states))
                                        @foreach ($states as $key => $val)
                                            <option value="{{ $val['states_name'] }}"
                                                {{ old('states') == $val['states_name'] ? 'selected' : '' }}>{{ $val['states_name'] }}
                                            </option>
                                        @endforeach
                                    @endif
                                </select>
